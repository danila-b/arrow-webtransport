...
describe('createAppLayout', () => {
  ...
  it('creates all expected elements', () => {
    ...
    expect(els.transportPicker).toBeInstanceOf(HTMLFieldSetElement);
  ...
  it('attaches elements to root', () => {
    ...
    expect(root.querySelector('fieldset.transport-picker')).not.toBeNull();
  ...
  it('has three transport radio buttons with webtransport selected by default', () => {
    const els = createAppLayout(root);
...
describe('getSelectedTransport', () => {
  it('returns the selected transport id', () => {
    const root = document.createElement('div');
    const els = createAppLayout(root);
    const radios = els.transportPicker.querySelectorAll<HTMLInputElement>('input[type="radio"]');
    radios[1].checked = true;
    expect(getSelectedTransport(els.transportPicker)).toBe('http2-arrow');
  });
});
